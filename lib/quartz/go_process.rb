class Quartz::GoProcess
  ...
  def initialize(opts)
    ...
    if opts[:file_path]
      ...
      raise BadConfig, 'Missing go binary'
    end
  ...
  def compile_and_run(path)
    ...
    unless system('go', 'build', '-o', @temp_file_path, path)
      raise BadConfig, 'Go compilation failed'
    end
  ...
  def block_until_server_starts
    ...
    loop do
      raise Quartz::Exception, 'RPC server not starting' if retries > max_retries
  ...
  def get_metadata
    ...
    if response['error']
      raise Quartz::Exception, "Metadata error: #{read['error']}"
    end
